...
class FreeGamesView:
    async def send_games_embeds(
        self, current_free_games: list[FreeGame], channel: discord.abc.Messageable
    ) -> None:
        """
    ...
    def make_game_embed(self, game: FreeGame) -> discord.Embed:
        embed = discord.Embed(
            title=f"{game.title}", description=f"{game.description} {game.url}"
        )
