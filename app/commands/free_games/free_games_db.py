...
class FreeGameDB:
    ...
    async def update_free_games(self) -> list[FreeGame]:
        ...
        async with in_transaction() as connection:
            current_free_games = (
                await FreeGame.filter(end_free_date=None).using_db(connection).all()
            )
            ...
            for known_free_game in current_free_games:
                if known_free_game.title not in new_free_titles:
                    known_free_game.end_free_date = datetime.now(tz=UTC)
    ...
    def __get_free_games(self) -> list[dict]:
        new_free_games = []
        free_games = self.api.get_free_games()["data"]["Catalog"]["searchStore"][
            "elements"
        ]
        ...
        for game in free_games:
            ...
            nested_promotions = game["promotions"]["promotionalOffers"][0][
                "promotionalOffers"
            ]
    ...
    def get_game_url(game: dict) -> str:
        try:
            ...
            page_slug = (
                game["productSlug"]
                or game["catalogNs"]["mappings"][0]["pageSlug"]
                or game["offerMappings"][0]["pageSlug"]
            )
            return (
                f"\n[**Link**](https://store.epicgames.com/en-US/p/{page_slug})"
                if page_slug
                else ""
            )
        except IndexError:
