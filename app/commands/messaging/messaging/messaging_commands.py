from datetime import datetime, timedelta

import discord
from discord import app_commands
from discord.ext import commands

from app.commands.messaging.messaging.messaging_model import (
    annoy_internal,
    dm_spam_internal,
    get_attention_internal,
)
from app.commands.messaging.messaging.MessagingInfo import MessagingInfo
from app.core.elgatron import Elgatron
from app.utilities.transformers import IntervalTransformer


class MessagingCommands(commands.Cog):
    def __init__(self, bot: Elgatron):
        self.bot: Elgatron = bot

    @app_commands.command(
        name="dm_spam",
        description="Annoy someone as many times as you would like with a given interval!",
    )
    async def dm_spam(
        self,
        ctx,
        user: discord.User,
        message: str,
        amount: discord.app_commands.Range[int, 1, None],
        interval: app_commands.Transform[timedelta, IntervalTransformer],
    ):
        messaging_info = MessagingInfo(
            dm_spam_internal,
            user,
            message,
            amount,
            ctx.channel,
            datetime.now() + timedelta(seconds=1),
            interval,
            self.bot.scheduler,
            self.bot.active_commands,
        )
        messaging_info.start_job()
        await ctx.response.send_message(
            embed=messaging_info.make_embed(), ephemeral=True, delete_after=10
        )

    @app_commands.command(
        name="get_attention",
        description="Ping someone once every 10 seconds 100 times or until they react",
    )
    async def get_attention(
        self,
        ctx,
        target: discord.User | discord.Role | None,
        message: str = "WAKE UP WAKE UP WAKE UP WAKE UP WAKE UP WAKE UP",
        amount: app_commands.Range[int, 1, None] = 100,
        interval: app_commands.Transform[timedelta, IntervalTransformer] = timedelta(
            seconds=10
        ),
    ):
        messaging_info = MessagingInfo(
            get_attention_internal,
            target,
            message,
            amount,
            ctx.channel,
            datetime.now() + timedelta(seconds=1),
            interval,
            self.bot.scheduler,
            self.bot.active_commands,
        )
        messaging_info.start_job()
        await ctx.response.send_message(
            embed=messaging_info.make_embed(), ephemeral=True, delete_after=10
        )

    @app_commands.command(
        name="annoy",
        description="Spam a message at someone!",
    )
    async def annoy(
        self,
        ctx,
        message: str,
        amount: app_commands.Range[int, 1, None],
        interval: app_commands.Transform[timedelta, IntervalTransformer],
        target: discord.User | discord.Role | None = None,
    ):
        messaging_info = MessagingInfo(
            annoy_internal,
            target,
            message,
            amount,
            ctx.channel,
            datetime.now() + timedelta(seconds=1),
            interval,
            self.bot.scheduler,
            self.bot.active_commands,
        )
        messaging_info.start_job()
        await ctx.response.send_message(
            embed=messaging_info.make_embed(), ephemeral=True, delete_after=10
        )


async def setup(bot: Elgatron):
    await bot.add_cog(MessagingCommands(bot), guild=discord.Object(id=bot.guild_id))
