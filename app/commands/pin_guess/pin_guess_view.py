...
class PinView(discord.ui.View):
    ...
    def make_first_embed(self) -> discord.Embed:
        ...
        embed: discord.Embed = discord.Embed(
            title="", description=self.pin.message_content, color=discord.Color.yellow()
        )
        # result of deliberation
        title = random.choices(
            ["Guess the pin!", "who dunnit?"], weights=[0.9, 0.1], k=1
        )[0]
        embed.set_author(
            name=title,
    ...
    def make_sinner_embed(self) -> discord.Embed:
        ...
        embed: discord.Embed = discord.Embed(
            title="", description=self.pin.message_content, color=discord.Color.green()
        )

        embed.set_author(
            name=f"{self.pin.user_name} on {date.strftime('%d/%m/%Y')}",
            icon_url=icon_url,
            url=url,
        )
        return embed
...
class TempPinView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Reveal the sinner!", style=discord.ButtonStyle.success, disabled=True
    )
    async def reveal_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
