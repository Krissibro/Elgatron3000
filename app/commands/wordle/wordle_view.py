...
from app.commands.wordle.wordle_logic import (
    HAIR,
    THIN,
    pad_wordle_letters,
    wordle_logic,
...
class WordleView:
    async def make_wordle_embed(self, game: WordleGame) -> discord.Embed:
        if game.is_finished():
            ...
            if time_taken is not None:
                embed.add_field(
                    name="Time spent:   ",
                    value=f"{timedelta_format(time_taken)}",
                    inline=False,
                )
        ...
        fields, known_letters, unknown_letters = self.process_guesses(
            game.guesses, game.word
        )
        for field in fields:
    ...
    def process_guesses(
        self, guesses: Iterable[WordleGuess], solution: str
    ...
    def make_stats_embed(self, stats: WordleStats) -> discord.Embed:
        ...
        embed.add_field(
            name="Wins",
            value=f"{stats.total_wins} ({percentage_wins:.2f}%)",
            inline=True,
        )
        embed.add_field(
            name="\t Number of guesses", value=stats.total_guesses, inline=True
        )
        embed.add_field(
            name="Average guesses", value=f"{average_guesses:.3f}", inline=True
        )
        embed.add_field(name="Current streak", value=stats.win_streak, inline=True)
        embed.add_field(
            name="Longest streak", value=stats.longest_win_streak, inline=True
        )
        embed.add_field(
            name="Fastest win",
            value=f"{timedelta_format(stats.fastest_win)}",
            inline=True,
        )
    ...
    def add_guess_hist(self, embed: discord.Embed, stats: WordleStats) -> discord.Embed:
        ...
        sorted_guess_distribution = sorted(
            stats.guess_distribution.items(), key=lambda key: int(key[0])
        )
        for guess_amount, guess_count in sorted_guess_distribution:
            ...
            embed.add_field(
                ...
                value=square_count * ":blue_square:"
                if square_count > 0
    ...
    @staticmethod
    def format_available_letters(
        known_letters: set[str], unknown_letters: set[str]
    ) -> str:
        sorted_known_letters = sorted(known_letters)
        sorted_available_letters = sorted(unknown_letters)
...
class WordleFinishedController(discord.ui.View):
    ...
    async def prev_button(
        ...
        await interaction.response.edit_message(
            embed=embed, view=WordleFinishedController(prev_game, self.view)
        )
    ...
    async def next_button(
        ...
        await interaction.response.edit_message(
            embed=embed, view=WordleFinishedController(next_game, self.view)
        )
